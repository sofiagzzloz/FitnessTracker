...
def list_exercises(
   ...
   return svc.list_exercises(db=db, user_id=user.id, q=q, category=category, limit=limit, offset=offset)
...
def update_exercise(
   ...
   return svc.update_exercise(db=db, user_id=user.id, exercise_id=exercise_id, payload=payload)
