...
async def external_browse(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Adapter error: {type(e).__name__}: {e}"
        )
...
def import_exercise(
    ...
    # ----- Link muscles to this exercise (dedupe by muscle) -----
    prim_slugs = set(muscles.get("primary") or [])
    sec_slugs = set(muscles.get("secondary") or [])
    # prefer primary role when slug appears in both
    ordered: list[tuple[str, str]] = [(s, "primary") for s in sorted(prim_slugs)] + [
        (s, "secondary") for s in sorted(sec_slugs - prim_slugs)
    ]
    added_ids: set[int] = set()
    for slug, role in ordered:
        m = existing.get(slug)
        if not m:
            continue
        if m.id in added_ids:
            continue
        added_ids.add(m.id)
        session.add(ExerciseMuscle(exercise_id=ex.id, muscle_id=m.id, role=role))  # type: ignore
    session.commit()
