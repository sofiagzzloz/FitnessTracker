...
def test_login_bad_credentials(client):
    # register a user
    client.post("/api/auth/register", json={"email": "bad@ex.com", "password": "goodpass"})
    # wrong password
    r = client.post("/api/auth/login", json={"email": "bad@ex.com", "password": "WRONG"})
    assert r.status_code == 401
...
def test_per_user_isolation_on_exercises(client):
    ...
    r = client.post("/api/exercises", json={"name": "Alice Only Curl", "category": "strength"})
    assert r.status_code in (200, 201)
...
def test_logout_clears_cookie_and_me_requires_auth(client):
    ...
    assert r.status_code == 200
    assert "access_token" in client.cookies or "session" in client.cookies  # support either name
...
def test_me_with_bad_token_returns_401(client):
    ...
    client.cookies.set("access_token", "definitely.not.valid", domain="testserver.local", path="/")
    client.cookies.set("session", "definitely.not.valid", domain="testserver.local", path="/")
    r = client.get("/api/auth/me")
    assert r.status_code == 401
...
def test_html_pages_require_auth(client):
    ...
    # Login
    client.post("/api/auth/register", json={"email":"hp@example.com","password":"secret123"})
    client.post("/api/auth/login", json={"email":"hp@example.com","password":"secret123"})
