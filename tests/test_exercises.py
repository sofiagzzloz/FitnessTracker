...
def test_exercises_name_and_category_filters(client):
    ...
    client.post(
        "/api/exercises", json={"name": "Shoulder Press", "category": "strength"}
    )
...
def test_delete_exercise_in_use_returns_409_and_usage(client):
    ...
    # exercise
    ex = client.post(
        "/api/exercises", json={"name": "Row Machine", "category": "strength"}
    ).json()
    # workout template
    ...
    ir = client.post(
        ...
        json={"exercise_id": ex["id"], "planned_sets": 3, "planned_reps": 10},
    )
...
def test_exercise_validation_and_duplicates(client):
    ...
    # create
    r = client.post(
        "/api/exercises", json={"name": "Bench Press", "category": "strength"}
    )
    assert r.status_code in (200, 201)
...
def test_exercises_per_user_isolation(client):
    ...
    r = client.post(
        "/api/exercises", json={"name": "Alice Only Move", "category": "strength"}
    )
    assert r.status_code in (200, 201)
    client.post("/api/auth/logout")
...
def test_delete_exercise_referenced_by_session_returns_409(client):
    ...
    tpl_id = wr.json()["id"]
    client.post(
        f"/api/workouts/{tpl_id}/items",
        json={"exercise_id": ex_id, "planned_sets": 3, "planned_reps": 10},
    )
    ...
    sr = client.post(
        "/api/sessions",
        json={"date": today, "title": "S409", "workout_template_id": tpl_id},
    )
